...
# 인덱스가 문자열일 때는 칼럼의 이름이 ‘2019-06’인 칼럼을 인덱싱하기 때문에 에러가 발생하지만, 인덱스를 DatetimeIndex로 변경해주면 특정 시간을 샘플링하는 기능을 사용할 수 있습니다
...
def split_xy(dataset, time_steps, y_column):
    ...
    for i in range(len(dataset)):
        ...
        if y_end_number > len(dataset):
            break
        tmp_x = dataset[i+1:x_end_number,1: ]                     #  0 : 5 , 1:
        tmp_y = dataset[x_end_number : y_end_number, -1]        #  5 : 7 , -1
        x.append(tmp_x)
        y.append(tmp_y)
    return np.array(x), np.array(y)

x1_ss, y1_ss = split_xy(x11, 5, 2)
x2_ki, y2_ki = split_xy(x22, 5, 2)
