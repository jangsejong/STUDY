...
samsung1 = samsung.drop(range(80,1120), axis=0)
kiwoom1 = kiwoom.drop(range(80,1060), axis=0)
...
x1 = samsung2.drop(columns=['일자','Unnamed: 6','등락률', '고가', '시가', '금액(백만)', '전일비', '신용비', '개인', '외인(수량)', '프로그램', '외인비', '종가', '기관', '저가'], axis=1) 
x2 = kiwoom2.drop(columns =['일자','Unnamed: 6','등락률', '고가', '시가', '금액(백만)', '전일비', '신용비', '개인', '외인(수량)', '프로그램', '외인비', '종가', '기관', '저가'], axis=1) 
x1 = np.array(x1)
x2 = np.array(x2)
...
def split_xy3(dataset, time_steps, y_column):                 
    ...
    for i in range(len(dataset)):
        ...
        y_end_number = x_end_number + y_column - 0    # 5 + 2 -0 = 7
        
        if y_end_number > len(dataset):
            break
        tmp_x = dataset[i:x_end_number, :-1]                # 0 : 5
        tmp_y = dataset[x_end_number-1:y_end_number, 0]    # 5 : 7 , 0
        x.append(tmp_x)
        y.append(tmp_y)
...
# xx1 = samsung2.drop(columns=['일자','Unnamed: 6','등락률', '고가', '시가', '금액(백만)', '전일비', '신용비', '개인', '외인(수량)', '프로그램', '외인비', '종가', '기관', '저가', '외국계', '거래량'], axis=1) 
# xx2 = kiwoom2.drop(columns =['일자','Unnamed: 6','등락률', '고가', '시가', '금액(백만)', '전일비', '신용비', '개인', '외인(수량)', '프로그램', '외인비', '종가', '기관', '저가', '외국계', '거래량'], axis=1) 
