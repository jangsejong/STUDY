...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    gbr = GradientBoostingRegressor(random_state = 42, max_depth = 4, learning_rate = 0.05, n_estimators = n_estimators)
    gbr.fit(tr_x, tr_y)
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    ngb = NGBRegressor(random_state = 42, n_estimators = n_estimators, verbose = 0, learning_rate = 0.03)
...
submission.to_csv('./././_data/dacon/house/0204_5.csv', index = False)
