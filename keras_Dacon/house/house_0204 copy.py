...
train=pd.read_csv('./././_data/dacon/house/train.csv')
test=pd.read_csv('./././_data/dacon/house/test.csv')
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    gbr = GradientBoostingRegressor(random_state = random_state, max_depth = 4, learning_rate = 0.05, n_estimators = n_estimators)
    gbr.fit(tr_x, tr_y)
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    cb.fit(tr_data, eval_set = val_data, early_stopping_rounds = 4000, verbose = 1000)
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    ngb = NGBRegressor(random_state = random_state, n_estimators = n_estimators, verbose = 0, learning_rate = 0.03)
...
submission.to_csv('./././_data/dacon/house/0204_4.csv', index = False)
