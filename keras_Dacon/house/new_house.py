...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    cb = CatBoostRegressor(depth = 4, random_state = random_state, loss_function = 'MAE', n_estimators = n_estimators)
    cb.fit(tr_data, eval_set = val_data, early_stopping_rounds = 1000, verbose = 100)
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    ngb.fit(tr_x, tr_y, val_x, val_y, early_stopping_rounds = 100)
...
for n, (tr_idx, val_idx) in enumerate(kf.split(X, y)) :
    ...
    xgb = XGBRegressor(random_state = 42, criterion = 'mae', n_estimators = 15000)
    xgb.fit(tr_x, tr_y, early_stopping_rounds = 100, eval_set = val_data)
...
(ngb_pred + cb_pred + rf_pred +  xgb_pred) / 4

submission['target'] = np.expm1((ngb_pred + cb_pred + rf_pred + xgb_pred) / 4)

submission.to_csv(path +'1more_2.csv', index = False)
